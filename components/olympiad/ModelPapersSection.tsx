      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-col items-end gap-3">
          ...
          <a
            href={paper.fileUrl}
            download
            className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-xs font-semibold text-primary-foreground transition-all hover:brightness-105"
          >
                ...
                <div className="space-y-3">
                  {grouped[year].map((p, i) => (
                    <FadeIn key={`${p.title}-${i}`} delay={i * 0.06}>
                      <PaperRow paper={p} colorVar={olympiad.colorVar} />
                    </FadeIn>
                  ))}
                </div>
              </div>
