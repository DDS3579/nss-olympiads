...
import { Motif, SectionHeading } from "./primitives";
...
export function Constellation({ olympiad }: { olympiad: Olympiad }) {
  ...
  const { focusedTopicId, focusTopic } = useOlympiadProgress();
  ...
  const isEdgeActive = (e: { from: string; to: string }) =>
    ...
    (focusedTopicId !== null && (e.from === focusedTopicId || e.to === focusedTopicId));

  const isNodeDimmed = (id: string) => {
    if (!focusedTopicId) return false;
    if (id === focusedTopicId) return false;
    return !edges.some(
      (e) =>
        (e.from === focusedTopicId && e.to === id) ||
        (e.to === focusedTopicId && e.from === id)
    );
  };
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        >
          <div className="relative aspect-[16/9] lg:aspect-[16/8]">
              ...
              {edges.map((e, i) => {
                ...
                return (
                  <motion.line
                    ...
                    transition={{ duration: reduce ? 0 : 0.9, delay: reduce ? 0 : 0.15 * i, ease: "easeOut" }}
            ...
            {topics.map((t, i) => {
              const active = hovered === t.id || focusedTopicId === t.id;
              const dimmed = isNodeDimmed(t.id);
              return (
                <motion.button
                  key={t.id}
                  initial={reduce ? false : { scale: 0, opacity: 0 }}
                  animate={inView ? { scale: 1, opacity: dimmed ? 0.35 : 1 } : {}}
                  transition={{ duration: reduce ? 0 : 0.4, delay: reduce ? 0 : 0.3 + 0.08 * i, ease: "easeOut" }}
                  onMouseEnter={() => setHovered(t.id)}
                  onMouseLeave={() => setHovered(null)}
                  onFocus={() => setHovered(t.id)}
                  onBlur={() => setHovered(null)}
                  onClick={() => focusTopic(t.id)}
                  aria-label={`Focus topic ${t.name}`}
                  aria-pressed={focusedTopicId === t.id}
                  className="group absolute flex -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-1.5 focus:outline-none"
                  style={{ left: `${t.x}%`, top: `${t.y}%`, opacity: dimmed ? 0.35 : 1, transition: "opacity 0.3s" }}
                >
                  <span className="relative block">
                    <span
                      className="absolute -inset-2 rounded-full transition-opacity"
                      style={{ background: `hsl(var(${colorVar}) / 0.15)`, filter: "blur(6px)", opacity: active ? 1 : 0 }}
                    />
                    <span
                      className={
                      ...
                      style={{
                        ...
                        boxShadow: active ? `0 0 16px hsl(var(${colorVar}) / 0.8)` : `0 0 12px hsl(var(${colorVar}) / 0.6)`,
                      }}
                    />
                    {focusedTopicId === t.id && (
                      <span
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 text-[11px] text-muted-foreground">
            <span className="uppercase tracking-[0.2em]">Skill map · {topics.length} nodes</span>
            <span>{focusedTopicId ? "Click the node again to unfocus" : "Click a node to focus its topic"}</span>
          </div>
        </div>
        ...
        <div className="md:hidden">
          <ol className="relative ml-2 space-y-6 border-l border-border pl-6">
            {topics.map((t, i) => {
              const isFocused = focusedTopicId === t.id;
              return (
                <FadeIn key={t.id} delay={i * 0.06}>
                  <li className="relative">
                    <span
                      ...
                      style={{
                        background: `hsl(var(${colorVar}))`,
                        boxShadow: isFocused ? `0 0 12px hsl(var(${colorVar}) / 0.8)` : "none",
                      }}
                    />
                    <button onClick={() => focusTopic(t.id)} className="group w-full text-left" aria-pressed={isFocused}>
                      <div className="flex items-center justify-between">
                        <span className="font-heading font-semibold text-foreground transition-colors group-hover:text-primary">
                          {t.name}
                        </span>
                        <span className="text-xs text-muted-foreground">{t.problems} problems</span>
                      </div>
                      <span className="text-xs text-muted-foreground">
                        {isFocused ? "Focused — tap to clear" : t.blurb}
                      </span>
                    </button>
                  </li>
                </FadeIn>
